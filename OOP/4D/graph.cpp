...
void Graph::InsertOperation(const string &node_name, const string &operation_type, 
    ...
    if (operation_type == "+") nodes_.insert(node_name, new Plus(node_name, nodes_[input1], nodes_[input2]));
    if (operation_type == "-") nodes_.insert(node_name, new Min(node_name, nodes_[input1], nodes_[input2]));
}
